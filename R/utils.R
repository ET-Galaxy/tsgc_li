...
#'
#' @export
write_results <- function(res, res.dir, Y=NA, n.ahead=NA, confidence.level=NA) {
  if (class(res)=="FilterResultsLI") {
    forecasts<-res$generate_forecast()
    write.csv(as.data.frame(forecasts$trend),
              file = file.path(res.dir, "trend.csv"))
    write.csv(as.data.frame(forecasts$seasonal),
              file = file.path(res.dir,"seasonal.csv"))
  } else {
    if (is.null(Y)||is.null(n.ahead)||is.null(confidence.level)) {
      stop("Y, n.ahead and confidence.level is required when res is of class 'FilterResults'")
    } else {
...
#'
#' @export
cross_val<-function(y,est.end.date,n.ahead,lags,totaldays=1,freq=1, vanilla=TRUE,
                    LeadIndCol=1){
  if (vanilla){
    alllags<-c(0,lags)
  }
